...
def bboxes_nms_fast(classes, scores, bboxes, nms_threshold=0.45,intersection_threshold = 0.6):
    """Apply non-maximum selection to bounding boxes.
    """
    keep_bboxes = np.ones(scores.shape, dtype=np.bool)
    for i in range(scores.size-1):
        if keep_bboxes[i]:
            # Computer overlap with bboxes which are following.
            overlap_intersection = bboxes_intersection(bboxes[i],bboxes[(i+1):])
            overlap_intersection_2 = bboxes_intersection(bboxes[(i + 1):],bboxes[i])
            overlap_intersection[overlap_intersection<overlap_intersection_2] = overlap_intersection_2[overlap_intersection<overlap_intersection_2]
            overlap = bboxes_jaccard(bboxes[i], bboxes[(i+1):])
            # Overlap threshold for keeping + checking part of the same class
            keep_overlap = np.logical_and(overlap < nms_threshold, overlap_intersection<intersection_threshold)
            keep_bboxes[(i+1):] = np.logical_and(keep_bboxes[(i+1):], keep_overlap)
