...
import ssd_tf_extended as tfe
from tensorflow.python.ops import control_flow_ops

from ssd_preprocessing import tf_image
...
def distort_color(image, color_ordering=0, fast_mode=True, scope=None):
    ...
    of color ops for each ssd_preprocessing thread.
...
def distorted_bounding_box_crop(image,
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes]):
        ...
        #begin :containing[offset_height,offset_width,0],size [target_height,target_width,-1],bbox shape [1,1,4]
        bbox_begin, bbox_size, distort_bbox = tf.image.sample_distorted_bounding_box(#bbox_begin shape=(3),bbox_size shape=(3,),distortbbox shape=(1,1,4)
                tf.shape(image),
                bounding_boxes=tf.expand_dims(bboxes, 0),
                min_object_covered=min_object_covered,
                aspect_ratio_range=aspect_ratio_range,# The cropped area of the image must have an aspect ratio = width / height within this range.
                area_range=area_range, #The cropped area of the image must contain a fraction of the supplied image within in this range.
                max_attempts=max_attempts,#Defaults to 100. Number of attempts at generating a cropped region of the image of the specified constraints. After max_attempts failures, return the entire image.
...
def preprocess_for_train(image, labels, bboxes,
    ...
    Args:
        ...
        output_height: The height of the image after ssd_preprocessing.
        output_width: The width of the image after ssd_preprocessing.
...
def preprocess_image(image,
    ...
    Args:
      ...
      output_height: The height of the image after ssd_preprocessing.
      output_width: The width of the image after ssd_preprocessing.
      is_training: `True` if we're ssd_preprocessing the image for training and
        `False` otherwise.
      resize_side_min: The lower bound for the smallest side of the image for
